...
Discovered by Dan Muey.

Due to how C<qw> w/ a comma as terminator and list context operators work this
can be done using any odd or even number of commas, fat commas (=>),
spaceships (<=>), and spaces between 2 outermost string terminator commas.

    my @data = qw,,=>,<=>,   , <=> ,, =>,;

An excellent opportunity for an ascii animation!

This constant has the value of am empty list.
