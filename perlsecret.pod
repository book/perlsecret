...
perform a given operation. These "secret operators" are not operators
in the sense that the Perl parser recognize them, but in the sense that
these Perl programmers have seen them often enough to recognize
them and eventually add them to their toolbox.

Just like some of the real Perl operators have nicknames (I<diamond>
(C<< <> >>), I<spaceship> (C<< <=> >>), I<fat comma> (C<< => >>), I<yada
yada> (C<...>)), the secret operators also have nicknames. Actually, they
B<have> to have a nickname, because they usually don't have a real name.
...
page: most of those "operators" are not suitable for production.
The secret operators are used by golfers, obfuscators and people who
like to have fun with their favorite programming language.

So try this at home, but it's not safe for work! C<;-)>
...
The I<baby cart> operator performs list interpolation in a string.
...
shortcut for C<scalar> (shaves 4 characters!)

This operator looks very much like the smart-match operator introduced
...
the Perl parser can't mix it up with the binary smart-match.
    ...
    Tue Aug 19 09:30:00 2010

=head2 Inchworm on a stick C< ~- >

Discovered by Ton Hospel, 2002.

High precedence decrement (on complement two architectures)

=head2 Space station C< -+- >

Discovered by Alistair McGlinchy, 2005.

a high precedence string numifier
...
Discovered by Rafaël Garcia-Suarez.
