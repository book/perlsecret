...
are not explicitely documented.
...
container, and can carry a payload (a filehandle, in this case).
...
The boolean list squssh operator to the rescue!
...
=head2 Non-numeric numeric constants C<< 9**9**9 >>, C<< -9**9**9 >> and C<< sin(9**9**9) >>

Generally the strings "NaN", "inf" and "-inf" when used in numeric
context (see the Venus operator) are interpreted as not a number,
positive and negative infinity respectively. However, some pre-5.10
versions of Perl will interpret them as 0.

=head2 Wink C<< -.0 >>

In integer arithmetic (e.g. a C<use integer> scope), 0 and -0 are the
same. However, elsewhere negative zero has interesting properties.
It evaluates to false (despite what Truth and Falsehood in L<perlsyn>
would have you believe) and is equal to zero when tested with C<< == >>;
however it is not equal to zero when tested with C<< eq >>.
...
Karasik, Abigail, Yitzchak Scott-Thoennes, Zefram, Tye McQueen, Maxim Vuets,
Aristotle Pagaltzis and the Fun With Perl mailing list.

=head1 REFERENCES

As will appear below, most of the secret operator action happens
...
The fwp post that presents the scredriver operators also presents a
