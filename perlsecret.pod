...
high-precedence increment (C<-~>) on integers (on a twos-complement
...
This golfing technique allows to get rid of a pair of parentheses:
    ...
    $y = ~-$x * 4; # identical (if $x is an integer) to $y = ($x-1)*4;

Here's the proof:

    $x - 1 == - ( -$x ) - 1

In twos complement architectures, to get the negation of a number,
...
by its counterpart, the spacestation does B<not> numify I<strings starting with a minus>
...
to C<< <> >> on its left, thus matching on a single ligne of input.
...
Rafaël Garcia-Suarez, Andreas J. König, Andy Armstrong,
and the Fun With Perl mailing list for inspiration, suggestions and pacthes.
...
Larry Wall discovers the babycart and describes it as "a way to
